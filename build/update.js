...
game.update = function() {
  var a, b, bab, bb, c, fir, g, gi, i, j, k, l, m, mm, n, num, o, p, pr, quakeNum, quakeTime, randomNum, randomSupply, ref, ref1, rot, ss, t;
  ...
  if (game.lworld === 'go') {
    if (game.opacityGoBlack > 0) {
      ...
      if (game.opacityGoBlack > 0.5) {
        ...
        while (i < game.whiteLine.length) {
          ...
          if (game.whiteLine[i].remove === true) {
    ...
    if (game.opacityGoBlack < 0.5 && game.goIntroductionImageheigh < 253) {
      ...
      if (game.EndlessMode) {
        ...
        if (game.nextEnemy < 40) {
          if (game.gameTimeIn < 0.5) {
            ...
            game.nextEnemy = Math.random() * 170 + 40;
          }
          if (game.gameTimeIn >= 0.5 && game.gameTimeIn < 1) {
            ...
            game.nextEnemy = Math.random() * 100 + 40;
          }
          if (game.gameTimeIn >= 1) {
            ...
            game.nextEnemy = Math.random() * 60 + 40;
          }
        }
      }
      game.gameTimeIn = (new Date().getTime() - game.gameTime) / 60000;
      if (game.energyValue > 0 && game.energyValue < 400) {
        game.energyValue -= 2;
      ...
      if (!game.bossOneAllow) {
        if (!game.EndlessMode) {
          ...
          if (game.nextEnemy < 40) {
            ...
            } else if (game.gameTimeIn >= 0.5 && game.gameTimeIn < 1) {
              game.enemyArray.push(new game.enemy('one'));
              game.enemyArray.push(new game.enemy('two'));
        ...
        if (game.bossOne.ballone) {
          ...
          if (game.bossOneBallTime % 11 === 0) {
            if (game.bossOneBallOneAllow) {
              game.bossOneBallArray.push(new game.bossBall(game.bossOne.x - 50, game.bossOne.y + 60, 2));
              game.bossOneBallArray.push(new game.bossBall(game.bossOne.x - 50, game.bossOne.y - 60, -2));
            }
          }
          if (game.bossOneBallTime % 99 === 0) {
            if (game.bossOneBallOneAllow) {
              game.bossOneBallOneAllow = false;
            } else {
              game.bossOneBallOneAllow = true;
            }
        ...
        while (bb < game.bossOneBallArray.length) {
          ...
          if (game.collide(game.bossOneBallArray[bb], game.spaceShip, true)) {
            if (game.quakeAllow) {
              ...
              quakeNum = 45;
              quakeTime = setInterval(function() {
                if (quakeNum % 5 === 0) {
                  game.bImgY += 4;
                } else {
                  game.bImgY -= 4;
                ...
                if (quakeNum < 0) {
                  ...
                  return clearInterval(quakeTime);
                }
              }, 130);
            }
